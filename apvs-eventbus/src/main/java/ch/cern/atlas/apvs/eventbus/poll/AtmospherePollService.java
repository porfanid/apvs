...
public class AtmospherePollService extends AbstractRemoteServiceServlet
	...
	private static final long serialVersionUID = -3041965687571614531L;
	...
	protected SuspendInfo suspend() {
		...
		if (atm == null) {
			throw new UnexpectedException(
					"Failed to find Atmosphere resource have you setup Atmosphere?",
					null);
		}
	...
	static void writeResponse(AtmosphereResource resource, Object message)
			throws IOException {
		try {
			RPCRequest rpcRequest = (RPCRequest) resource.getRequest()
					.getAttribute(AtmospherePollService.GWT_REQUEST);
	...
	static String encodeResponse(RPCRequest rpcRequest, Object message)
		...
		responsePayload = RPC.encodeResponseForSuccess(rpcRequest.getMethod(),
				message, rpcRequest.getSerializationPolicy(),
				rpcRequest.getFlags());
	...
	protected void onAfterRequestDeserialized(RPCRequest rpcRequest) {
		getAtmosphereResource().getRequest().setAttribute(GWT_REQUEST,
				rpcRequest);
	}

	private AtmosphereResource getAtmosphereResource() {
		AtmosphereResource atm = (AtmosphereResource) getThreadLocalRequest()
	...
	public final void processPost(HttpServletRequest request,
		...
		if (!isSuspended()) {
			...
			// Write the response.
			//
			writeResponse(request, response, getServletContext(),
					responsePayload);
		}
	}
	...
	protected static void writeResponse(HttpServletRequest request,
		...
		RPCServletUtils.writeResponse(context, response, responsePayload,
				gzipEncode);
	}

...
public class TimeSlider extends SliderBar {
	...
	interface ImagesTimerSlider extends ClientBundle {
	...
	ImagesTimerSlider images = GWT.create(ImagesTimerSlider.class);

	private boolean timerOn = false;

	public TimeSlider(int maxValue, String width) {
		super(new TimeSliderPresenter());
	...
	// NOTE: below is copied from SliderBarHorizontal
	...
	protected int getBarHeight() {
		...
		for (int i = 0; i < orderedWidgets.size(); i++) {
			if (orderedWidgets.get(i) == scale) {
				...
				barHeight = Math.max(barHeight, orderedWidgets.get(i)
						.getOffsetHeight());
			}
		}
		barHeight = Math.max(barHeight, scaleSize);
	...
	public void drawScrollBar(int barWidth) {
		...
		absPanel.setWidgetPosition(drag, getScaleLeft(orderedWidgets),
				dragTopPosition);
	}
	...
	public int getScaleTouchPosition(
			@SuppressWarnings("rawtypes") MouseEvent event) {
		return event.getRelativeX(this.getElement()) - startPosition
				- drag.getOffsetWidth() / 2;
	}
	...
	public void putMark(Mark mark, int markPosition) {
		...
		this.absPanel.add(mark,
				this.startPosition + markPosition + drag.getOffsetWidth() / 2,
				markY);
	}
	...
	public void setValue(int value, boolean fireEvent) {
		((TimeSliderPresenter)presenter).setValue(value, fireEvent);
	}
}
